...
def seed_equipos_lb():
    try:
        ...
        if cur.fetchone()["c"] == 0:
            for e in LB_EQUIPOS_SEED:
                cur.execute("INSERT INTO equipos (id,area_id,categoria,descripcion,codigo,intervenciones) VALUES (%s,%s,%s,%s,%s,%s) ON CONFLICT (id) DO NOTHING",
                    (e["id"],"LB",e["categoria"],e["descripcion"],e.get("codigo",""),json.dumps(e.get("intervenciones",[]))))
            conn.commit()
        cur.close(); conn.close()
    except Exception as ex:
        print("seed_equipos_lb error:", ex)
...
def get_equipos(area_id: str):
    try:
        conn = get_conn(); cur = conn.cursor()
        cur.execute("SELECT * FROM equipos WHERE area_id=%s ORDER BY id", (area_id,))
        rows = cur.fetchall(); cur.close(); conn.close()
        return [dict(r) for r in rows]
    except Exception:
        return []
...
def create_equipo(data: EquipoIn):
    try:
        conn = get_conn(); cur = conn.cursor()
        if data.id and data.id > 0:
            new_id = data.id
        else:
            cur.execute("SELECT COALESCE(MAX(id),0)+1 as nid FROM equipos")
            new_id = cur.fetchone()["nid"]
        cur.execute("""INSERT INTO equipos (id,area_id,categoria,descripcion,codigo,intervenciones)
                       VALUES (%s,%s,%s,%s,%s,%s)
                       ON CONFLICT (id) DO UPDATE SET
                         area_id=EXCLUDED.area_id, categoria=EXCLUDED.categoria, descripcion=EXCLUDED.descripcion,
                         codigo=EXCLUDED.codigo, intervenciones=EXCLUDED.intervenciones
                       RETURNING *""",
                    (new_id, data.area_id, data.categoria, data.descripcion, data.codigo, json.dumps(data.intervenciones)))
        row = dict(cur.fetchone()); conn.commit(); cur.close(); conn.close()
        return row
    except Exception as e:
        raise HTTPException(500, str(e))
...
function getEquipos(areaId){
  return _equiposCache[areaId] || [];
}

async function fetchEquipos(areaId){
  try{
    const r = await fetch('/api/equipos/'+areaId);
    const d = await r.json();
    _equiposCache[areaId] = Array.isArray(d) ? d : [];
  }catch(e){ _equiposCache[areaId] = _equiposCache[areaId] || []; }
  return _equiposCache[areaId];
}

async function saveEquipo(areaId,eq){
  try{
    const r = await fetch('/api/equipos',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({...eq,area_id:areaId})});
    if(!r.ok) throw new Error('Error al guardar equipo');
  }catch(e){ alert('Error: '+e.message); throw e; }
  await fetchEquipos(areaId);
}

async function deleteEquipo(areaId,eqId){
  try{
    await fetch('/api/equipos/'+eqId,{method:'DELETE'});
...
async function showModal_area(areaId,eqId){
  if(!_equiposCache[areaId]) await fetchEquipos(areaId);
...
async function cargarEquiposReg(){
  const aId = document.getElementById('reg-area').value || currentArea;
  if(!_equiposCache[aId]) await fetchEquipos(aId);
